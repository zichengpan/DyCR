...
def set_up_datasets(args):
    ...
    if args.dataset == 'PlantVillage':
        import dataloader.plantvillage.plantvillageClass as Dataset
        args.base_class = 23
        args.num_classes=38
        args.way = 5
        args.shot = 5
...
def get_new_dataloader(args,session):
    ...
    if args.dataset == 'PlantVillage':
        trainset = args.Dataset.PlantVillage(root=args.dataroot, train=True, index_path=txt_path)
    ...
    if args.dataset == 'mini_imagenet':
        testset = args.Dataset.MiniImageNet(root=args.dataroot, train=False,
                                      index=class_new)
    if args.dataset == 'PlantVillage':
        testset = args.Dataset.PlantVillage(root=args.dataroot, train=False, index=class_new)
